...
main()
{
	...
	lngErrorcode = ePut (lngHandle, LJ_ioPUT_CONFIG, LJ_chAIN_RESOLUTION, 16, 0);
	ErrorHandler(lngErrorcode, __LINE__, 0);

	//Configure the analog input range on channels 2 and 3 for bipolar gain=1.
	lngErrorcode = ePut (lngHandle,  LJ_ioPUT_AIN_RANGE, 2, LJ_rgBIP5V, 0);
	ErrorHandler(lngErrorcode, __LINE__, 0);

	lngErrorcode = ePut (lngHandle,  LJ_ioPUT_AIN_RANGE, 3, LJ_rgBIP5V, 0);
	ErrorHandler(lngErrorcode, __LINE__, 0);
